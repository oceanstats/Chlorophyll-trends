...
for(i in 1:length(time)){wsp[,,i]<- rbind(wsp[ind==F,,i],wsp[ind==T,,i])}
...
wsptemp <- as.vector(aperm(wsp,c(3,1,2)))

monthtemp <-as.factor(rep(c(9:12,rep(1:12,times=16)),times=(chl.dim[1]*chl.dim[2])))

spTmodelFull <- data.frame(chl=chltemp,Longitude=coordtemp[[2]],Latitude=as.vector(coordtemp[[3]]),site=sitetemp,TT=TT,SST=SSTtemp,M=monthtemp,wsp=wsptemp)
